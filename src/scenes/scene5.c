...
/*   Updated: 2017/03/24 01:55:22 by vchaillo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			load_scene5_objects(t_scene *scene)
{
	...
	add_object(scene, new_object(CYLINDER, new_cylinder(new_vector(0, 1, -0.3),
		...
		new_material(BASIC)));
	add_object(scene, new_object(SPHERE, new_sphere(new_vector(-5, 15, 10), 20, new_vector(-10, 1, 2), limits),
		new_color(BLUE), new_material(BASIC)));
	add_object(scene, new_object(PLANE, new_plane(new_vector(0, 1, 0), 0, CHECK, limits),
